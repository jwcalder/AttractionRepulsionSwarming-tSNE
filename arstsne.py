...
def init_bh_tsne(samples, workdir, no_dims=DEFAULT_NO_DIMS, initial_dims=INITIAL_DIMENSIONS, perplexity=DEFAULT_PERPLEXITY, theta=DEFAULT_THETA, randseed=EMPTY_SEED, verbose=False, use_pca=DEFAULT_USE_PCA, max_iter=DEFAULT_MAX_ITERATIONS,time_step=DEFAULT_TIME_STEP,theta1=DEFAULT_THETA1,theta2=DEFAULT_THETA2,alpha=DEFAULT_ALPHA,num_early=DEFAULT_NUM_EARLY):
    ...
    with open(path_join(workdir, 'data.dat'), 'wb') as data_file:
        ...
        data_file.write(pack('iiddiiddddi', sample_count, sample_dim, theta, perplexity, no_dims, max_iter, time_step, theta1, theta2, alpha, num_early))
        # Then write the data
...
def run_ars_tsne(data, no_dims=2, perplexity=50, theta=0.5, randseed=-1, verbose=False, initial_dims=50, use_pca=True, max_iter=1000,time_step=1,theta1=2.0,theta2=2.0,alpha=1,num_early=100):
    '''
    ...
    if child_pid == 0:
        ...
        init_bh_tsne(data, tmp_dir_path, no_dims=no_dims, perplexity=perplexity, theta=theta, randseed=randseed,verbose=verbose, initial_dims=initial_dims, use_pca=use_pca, max_iter=max_iter, time_step=time_step, theta1=theta1, theta2=theta2, alpha=alpha, num_early=num_early)
        os._exit(0)
    else:
        try:
...
def main(args):
    ...
    for result in run_ars_tsne(argp.input, no_dims=argp.no_dims, perplexity=argp.perplexity, theta=argp.theta, randseed=argp.randseed,
